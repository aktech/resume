...
\section{\faPieChart}{Experience}
  ...
  \resumeEntryStart
    ...
    \resumeItemListStart
    \resumeItem{Core maintainer of \textbf{Nebari}, an open-source, Kubernetes-native data-science platform; automated TLS, OIDC across AWS, GCP and Azure.}
    \resumeItem{Optimised GPU kernels in population-genetics toolkit \textbf{sgkit}, delivering a \textbf{10× speed-up} and cutting cohort-analysis runtimes from hours to minutes (CUDA, Dask, RAPIDS).}
    \resumeItem{Led backend work on various client services (FastAPI, Flask, Django)}
    \resumeItem{Built a \textbf{JupyterHub extension} that launches sandboxed Streamlit, Gradio and Panel apps on Kubernetes cluster and local.}
...
\resumeEntryStart
  ...
  \resumeItemListStart
    \resumeItem{Architected a \textbf{multi-cloud CI/CD platform} that transparently schedules GitHub Actions on \textbf{6 providers} (AWS, GCP, Azure, OCI, DigitalOcean, OpenStack) using Celery for task queuing }
    \resumeItem{Serves over \textbf{100 000+ API requests daily} through \textbf{Cloudflare Workers}}
    \resumeItem{Built a real-time observability system via Beszel and led a 24×7 on-call rotation, sustaining \textbf{99.9 \%} platform uptime.}
    \resumeItem{Tech Stack: Python (FastAPI, Celery), Postgres, Redis, Terraform, GitHub Actions API, Cloudflare Workers.}
  \resumeItemListEnd
\resumeEntryEnd
  ...
  \resumeEntryStart
    ...
    \resumeItemListStart
      \resumeItem {Worked as contract Software Engineer for Aire Labs (Fintech), Turner \& Townsend, Cambridge Medical Robotics}
